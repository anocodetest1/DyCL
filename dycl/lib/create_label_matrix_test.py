...
def create_label_matrix_test(
    ...
    if other_coords.ndim == 3:
       label_matrix = torch.zeros((len(coords), other_coords.size(1)),dtype=torch.float)
       for i in range(len(coords)):
          layer = other_coords[i,:,:]
          for j in range(other_coords.size(1)):
               distances = distance(coords[i], layer[j]).m
