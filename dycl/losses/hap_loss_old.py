...
from dycl.losses.tools import reduce
...
class SmoothRankHAP(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.hierarchy_level != "MULTI":
            coords = coords[:, self.hierarchy_level:self.hierarchy_level+1]
        ...
        if ref_coords is None:
            ...
            assert coords.size(1) == ref_coords.size(1)
            if self.hierarchy_level != "MULTI":
                ref_coords = ref_coords[:, self.hierarchy_level:self.hierarchy_level+1]
        ...
        if (embeddings.size(0) == ref_embeddings.size(0)) and not force_general:
            ...
            target = lib.create_label_matrix(coords, ref_coords, dtype=torch.int64)
            relevances = relevance_fn(target).type(scores.dtype)  # / (labels.size(1) - 1)
            target = target.to(scores.device)
            relevances = relevances.to(scores.device)
...
class HAPLoss_old(SmoothRankHAP):
