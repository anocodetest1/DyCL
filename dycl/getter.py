...
from dycl import losses
from dycl import datasets
from dycl import models
from dycl import engine
from dycl.models import schedulers
...
class Getter:
    ...
    def get_dataset(self, transform, mode, platform_s, platform_d, config):
        if (config.name == "InShopDataset") and (mode == "test"):
            ...
            dataset = getattr(datasets, config.name)(
                transform=transform,
                mode=mode,
                platform_1=platform_s,
                platform_3=platform_d,
                **config.kwargs,
            )
            lib.LOGGER.info(dataset)
            return dataset
    ...
    def get_dataset_test(self, transform, mode, platform, platform_ref, config):
            dataset = getattr(datasets, config.name)(
                transform=transform,
                mode=mode,
                platform=platform,
